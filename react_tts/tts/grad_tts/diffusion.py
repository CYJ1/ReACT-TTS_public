...
from react_tts.tts.grad_tts.baseblock import (
    ResnetBlock,
    Residual,
    Block,
    Mish,
...
class Diffusion(pl.LightningModule):
    ...
    def reverse_diff(self, z, mask, mu, n_steps, stoc=False, spk=None):
        ...
        for i in range(n_steps):
            t = (1.0 - (i + 0.5) * h) * torch.ones(
                z.size(0), dtype=z.dtype, device=z.device
            )
            time = t.unsqueeze(-1).unsqueeze(1)
            ...
            if stoc:
                ...
                dxt_stoc = torch.randn(
                    z.size(), dtype=z.dtype, device=z.device, requires_grad=False
                )
    ...
    def compute_loss(self, x0, mask, mu, spk=None, offset=1e-5):
        t = torch.rand(
            x0.size(0), dtype=x0.dtype, device=x0.device, requires_grad=False
        )
