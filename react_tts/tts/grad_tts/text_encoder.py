...
from react_tts.tts.grad_tts.utils import sequence_mask, convert_pad_shape
...
class ConvReluNorm(pl.LightningModule):
    def __init__(
        self,
        in_channels,
        hidden_channels,
...
class TextEncoder(pl.LightningModule):
    def __init__(
        ...
        self.encoder = Encoder(
            n_channels + (spk_emb_dim if multi_spks else 0),
            filter_channels,
            n_heads,
            n_layers,
            kernel_size,
        ...
        self.proj_m = nn.Conv1d(
            n_channels + (spk_emb_dim if multi_spks else 0), n_feats, 1
        )
